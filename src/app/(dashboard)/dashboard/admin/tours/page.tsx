  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-slate-500 text-sm">{tours.length} Active Listings</div>
      </div>
      
      <div className="grid grid-cols-1 gap-4">
        {tours.map((tour) => (
          <div key={tour._id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                   Guide: <span className="text-indigo-600 font-medium">{tour.guide?.name || "Unknown"}</span> • ${tour.price}
                </p>
              </div>
            </div>
