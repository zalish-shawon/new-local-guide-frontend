    ...
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link href="/tours" className="text-slate-600 hover:text-indigo-600 font-medium transition">
              Explore Tours
            </Link>
            ...
            {user ? (
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  <span className="font-medium">{user.role === 'guide' ? 'Guide Portal' : 'My Trips'}</span>
                </Link>
                <button 
                  onClick={logout}
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 p-4 space-y-4 shadow-lg">
          <Link href="/tours" className="block text-slate-600 font-medium">Explore Tours</Link>
          {user ? (
            <>
              <Link href="/dashboard" className="block text-slate-600 font-medium">Dashboard</Link>
              <button onClick={logout} className="block w-full text-left text-red-500 font-medium">Logout</button>
            </>
          ) : (
            <>
